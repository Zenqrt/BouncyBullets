...
public final class GameMapManager {
    ...
    public void loadGameMaps(File mapFolder) {
        if (!mapFolder.exists() && !mapFolder.mkdirs())
            throw new IllegalStateException("Could not create map folder at " + mapFolder.getAbsolutePath());
        ...
        for (File directory : folders) {
            if (directory.isDirectory()) {
                ...
                gameMaps.put(directory.getName(), new GameMap(displayName, worldFolder, configuration));
            }
        }
    }
